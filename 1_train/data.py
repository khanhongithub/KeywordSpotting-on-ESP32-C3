...
def load_wav_file(wav_filename, desired_samples):
    ...
    decoded_wav = audio_ops.decode_wav(
        wav_file, desired_channels=1, desired_samples=desired_samples
    )
...
def calculate_features(
    ...
    Args:
        ...
        num_bins: The number of frequency bins wanted.
    ...
    if micro:
        ...
        features = tf.multiply(micro_frontend, (10.0 / 256.0))
        features = tf.expand_dims(tf.expand_dims(features, -1), 0)
    else:
...
class AudioProcessor:
    ...
    def get_data(
        ...
        Returns:
            TF dataset that will generate tuples containing an feature and corresponding label.
    ...
    def process_path(
        ...
        if use_background:
            ...
            background_offset = tf.random.uniform(
                shape=(),
                maxval=len(background_sample) - desired_samples,
                dtype=tf.int32,
            )
            background_clipped = background_sample[
